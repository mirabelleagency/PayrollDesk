...
def serialize_v2_payout(payout: Payout) -> dict:
    wallet = None
    if payout.model and payout.model.deleted_at is None:
        wallet = payout.model.crypto_wallet
    gross = payout.gross_amount if payout.gross_amount is not None else payout.amount
    net = payout.amount
    deduction = gross - net if gross is not None and net is not None else Decimal("0")
    if deduction < 0:
        deduction = Decimal("0")
    return {
        "id": payout.id,
        ...
        "amount": _money(net),
        "breakdown_source": "native" if payout.gross_amount is not None else "legacy_net_only",
        "status": payout.status,
        "notes": payout.notes,
        "superseded_at": None,
        "is_locked": payout.is_locked,
...
def serialize_v2_schedule_run(run: ScheduleRun) -> dict:
    ...
    return {
        ...
        "pay_config_id": run.pay_config_id,
